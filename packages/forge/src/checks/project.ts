...
function collectImportSpecifiers(sourceFile: ts.SourceFile): readonly string[] {
  ...
  const visit = (node: ts.Node): void => {
    ...
    if (ts.isCallExpression(node) && node.arguments.length === 1) {
      ...
      if ((dynamicImport || requireCall) && argument && ts.isStringLiteral(argument)) specifiers.add(argument.text);
    }
...
function collectStaticFetchUrls(sourceFile: ts.SourceFile): readonly string[] {
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) && node.expression.text === 'fetch' && node.arguments.length > 0) {
      const [argument] = node.arguments;
      if (argument && (ts.isStringLiteral(argument) || ts.isNoSubstitutionTemplateLiteral(argument))) urls.add(argument.text);
    }
    ts.forEachChild(node, visit);
  };
  visit(sourceFile);
...
function isDirectOrmImport(specifier: string, resolvedProjectPath: string | null): boolean {
  const packageImport = ORM_PACKAGES.some((packageName) =>
    specifier === packageName || specifier.startsWith(`${packageName}/`) || specifier.startsWith(packageName));
  const localImport =
...
function isApplicationFrameworkImport(specifier: string): boolean {
  return specifier === 'next' || specifier.startsWith('next/') || specifier === 'react' || specifier.startsWith('react/') || specifier === 'server-only' || specifier.startsWith('node:');
}
...
export function inspectSourceFile({ root, filePath, source }: SourceInspectionInput): readonly CheckFailure[] {
  ...
  const imports = collectImportSpecifiers(sourceFile).map((specifier) => ({ specifier, resolvedProjectPath: resolveProjectImport(root, filePath, specifier) }));
  ...
  if (projectFile.startsWith('src/app/')) {
    for (const reference of imports) {
      if (isDirectOrmImport(reference.specifier, reference.resolvedProjectPath)) add('APP_DIRECT_ORM_IMPORT', `A delivery réteg közvetlen ORM-importot használ: ${reference.specifier}`);
    }
    if (!clientComponent) {
  ...
  if (projectFile.includes('/application/')) {
    for (const reference of imports) {
      if (isApplicationFrameworkImport(reference.specifier)) add('APPLICATION_FRAMEWORK_IMPORT', `Az application réteg framework- vagy runtime-specifikus importot használ: ${reference.specifier}`);
      if (reference.resolvedProjectPath?.includes('/infrastructure/') || reference.resolvedProjectPath?.includes('/presentation/') || reference.resolvedProjectPath?.startsWith('src/composition/') || reference.resolvedProjectPath?.startsWith('src/app/')) {
        add('APPLICATION_OUTWARD_IMPORT', `Az application réteg kifelé mutató függőséget használ: ${reference.specifier}`);
      }
      if (isDirectOrmImport(reference.specifier, reference.resolvedProjectPath)) add('APPLICATION_ORM_IMPORT', `Az application réteg közvetlen ORM-importot használ: ${reference.specifier}`);
    }
  }

  if (clientComponent) {
    for (const reference of imports) {
      const serverImport = reference.specifier === 'server-only' || reference.specifier.startsWith('node:') || reference.specifier.includes('.server') || reference.resolvedProjectPath?.startsWith('src/composition/') === true || reference.resolvedProjectPath?.includes('/infrastructure/') === true || reference.resolvedProjectPath?.startsWith('src/platform/database/') === true || isDirectOrmImport(reference.specifier, reference.resolvedProjectPath);
      if (serverImport) add('CLIENT_SERVER_IMPORT', `A Client Component szerveroldali importot használ: ${reference.specifier}`);
    }
  }

  if (projectFile.startsWith('src/composition/') && !importsServerOnly) add('COMPOSITION_MISSING_SERVER_ONLY', 'A composition root fájlnak explicit server-only határt kell deklarálnia.');
  if (projectFile.includes('/infrastructure/') && imports.some(({ specifier }) => specifier.startsWith('node:')) && !importsServerOnly) add('NODE_ADAPTER_MISSING_SERVER_ONLY', 'A Node.js runtime API-t használó infrastruktúra-adapterből hiányzik a server-only import.');
...
async function checkCapabilities(root: string, manifest: WinzardManifest): Promise<readonly CheckFailure[]> {
  ...
  for (const capability of manifest.capabilities) {
    ...
    for (const dependency of definition.requires ?? []) {
      if (!enabled.has(dependency)) failures.push({ code: 'CAPABILITY_DEPENDENCY_MISSING', file: 'winzard manifest', message: `${capability} megköveteli ezt a capability-t: ${dependency}` });
    }
    for (const requiredPath of definition.all ?? []) {
      if (!(await exists(path.join(root, requiredPath)))) failures.push({ code: 'CAPABILITY_PATH_MISSING', file: requiredPath, message: `A(z) ${capability} capability kötelező útvonala hiányzik.` });
    }
    for (const alternatives of definition.any ?? []) {
      ...
      if (!present) failures.push({ code: 'CAPABILITY_PATH_MISSING', file: alternatives.join(' | '), message: `A(z) ${capability} capability egyik alternatív útvonala sem található.` });
    }
  }
  return failures;
}
...
async function checkNoStore(root: string, file: string, code: string): Promise<readonly CheckFailure[]> {
  try {
    ...
    return source.includes('no-store') ? [] : [{ code, file, message: 'A health válaszból hiányzik a no-store cache policy.' }];
  } catch (error) {
...
async function checkPrismaConfig(root: string): Promise<readonly CheckFailure[]> {
  ...
  if (/\benv\s*\(\s*['"]DATABASE_URL['"]\s*\)/u.test(source)) {
    failures.push({ code: 'PRISMA_CONFIG_EAGER_DATABASE_URL', file: selected.file, message: 'A Prisma Config ne használjon kötelező env(DATABASE_URL) hívást a generate-kompatibilis profilban.' });
  }
  if (!source.includes('process.env.DATABASE_URL')) {
    failures.push({ code: 'PRISMA_CONFIG_DATABASE_URL_FALLBACK', file: selected.file, message: 'A Prisma Config közvetlen process.env.DATABASE_URL hozzáférést használjon opcionális fallbackkel.' });
  }
  return failures;
}
...
export async function runProjectChecks(root = process.cwd()): Promise<readonly CheckFailure[]> {
  ...
  if (enabled.has('liveness')) failures.push(...(await checkNoStore(root, 'src/app/api/health/live/route.ts', 'LIVENESS_CACHE_POLICY')));
  if (enabled.has('database-readiness')) failures.push(...(await checkNoStore(root, 'src/app/api/health/ready/route.ts', 'READINESS_CACHE_POLICY')));
