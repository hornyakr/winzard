...
try {
  if (command === 'list') {
    await import('./cli');
    console.log([
      'delivery:list', 'delivery:inspect', 'delivery:check', 'http:contracts',
      'view:list', 'view:inspect', 'view:check', 'view:contracts', 'view:assets',
      'make:page', 'make:route-handler', 'make:action', 'make:operation',
      'make:vertical-slice', 'make:view',
    ].join('\n'));
  } else if (command === 'check' || command === 'security:check') await projectCheck();
  else if (!await viewCommand() && !await deliveryCommand()) await import('./cli');
} catch (error) {
