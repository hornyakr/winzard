...
describe('Forge view and presentation platform', () => {
  ...
  it('nem minősíti presentation nézetnek a Route Handlert', async () => {
    const root = await fixture();
    await file(root, 'src/app/api/health/ready/route.ts', "import { database } from '@/platform/database/client'; export function GET() { return Response.json({ ready: Boolean(database) }); }");

    expect((await buildViewInventory(root)).issues).not.toContainEqual(
      expect.objectContaining({ code: 'VIEW_DIRECT_ORM_IMPORT' }),
    );
  });
